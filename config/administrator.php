...
return array(
     ...
     *
     * @type array
     *  后台菜单数组，多为数组渲染结果为多级嵌套菜单
     *  数组里的值有三种类型：
     *  1.字符串 -- 子菜单的入口，不可访问
     *  2.模型配置文件 -- 访问 model_config_path 目录下的模型文件，如 users 访问的是users.php模型配置文件
     *  3.配置信息 -- 必须使用前缀 settings.  对应settings_config_path 目录下的文件，如：默认设置下，
     *  settings.site 访问的是  administrator/settings/site.php文件
     *  4.页面文件 -- 必须使用前缀page.  如： page.pages.analytics 对应 administrator/pages/analytics.php或者
     *  administrator/pages/analytics.blade.php 两种后缀名皆可
     *  示例：
     * 	array(
     ...
     *  权限控制的回调函数
     *  此函数需要返回true或false 用来检测当前用户是否有权限访问后台
     * true为通过，false会将页面重定向到login_path选项定义的URL中。
     * @type closure
     */
    'permission' => function () {
     ...
     *  使用布尔值来设定是否使用后台主页
     * 如值为true，将使用dashboard_view定义的视图文件渲染页面
     * 如值为false，将使用home_page定义的菜单条目来作为后台主页
     * @type bool
     */
    'use_dashboard' => false,
     ...
     *  用来作为后台主页的菜单条目，由use_dashboard选项决定
     * @type string
     */
    'home_page' => 'users',
     ...
     *  当选项permission 权限检测不通过时，会重定向用户到此处设置的路径
     * @type string
     */
    'login_path' => 'login',
     ...
     *  允许在登录成功后使用Session::get('redirect')将用户重定向到原本想要访问的后台页面
     * @type string
     */
